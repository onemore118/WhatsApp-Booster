          ...
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE && node.classList.contains('_2Ex_b')) {
                ...
                document.querySelector('.icon_btn').addEventListener('click', function() {
                        ...
                        const ps = input.querySelectorAll('p');
                        const messageList = [];
                        ...
                        for (const p of ps) {
                            ...
                            for(const span of spans) {
                              if(span.classList.contains('fe5nidar')) {
                                continue;
                              }
                        ...
                        if(messageList.length > 0) {
                            transaltion_area.textContent = ''
                            all_message = ''
    
                            const translator_svg = document.getElementById('transaltor_svg');
                            translator_svg.setAttribute("fill", "#ffb7da");
                            ...
                            port.onMessage.addListener((message) => {
                              ...
                              const timer = setInterval(() => {
                                if (index >= message.text.length) {
                                  ...
                                  transaltion_area.textContent += message.text.charAt(index);
                                  index++;
                                }
                              }, 50);

                              translator_svg.setAttribute("fill", "#000000");    
                              // 关闭连接
                              port.disconnect();
                            });
